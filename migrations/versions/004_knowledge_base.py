...
def upgrade() -> None:
    ...
    op.execute('CREATE POLICY tenant_isolation_policy ON knowledge_bases USING (tenant_id = current_setting(''app.current_tenant_id'', true)::uuid)')

    op.execute('ALTER TABLE knowledge_documents ENABLE ROW LEVEL SECURITY')
    op.execute('CREATE POLICY tenant_isolation_policy ON knowledge_documents USING (tenant_id = current_setting(''app.current_tenant_id'', true)::uuid)')

    op.execute('ALTER TABLE knowledge_chunks ENABLE ROW LEVEL SECURITY')
    op.execute('CREATE POLICY tenant_isolation_policy ON knowledge_chunks USING (tenant_id = current_setting(''app.current_tenant_id'', true)::uuid)')
