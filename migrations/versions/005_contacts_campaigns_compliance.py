...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.CheckConstraint("status IN ('active', 'inactive', 'blocked')", name='contact_status_check'),
    )
    op.create_index('ix_contacts_tenant_id', 'contacts', ['tenant_id'])
    op.create_index('ix_contacts_phone_tenant', 'contacts', ['tenant_id', 'phone_number'], unique=True)
    ...
    op.create_table(
        ...
        sa.Column('id', postgresql.UUID(as_uuid=True), primary_key=True),
        sa.Column('tenant_id', postgresql.UUID(as_uuid=True), sa.ForeignKey('tenants.id', ondelete='CASCADE'), nullable=False),
        sa.Column('contact_list_id', postgresql.UUID(as_uuid=True), sa.ForeignKey('contact_lists.id', ondelete='CASCADE'), nullable=False),
    ...
    op.create_index('ix_list_member_unique', 'contact_list_members', ['contact_list_id', 'contact_id'], unique=True)
    ...
    op.create_table(
        ...
        sa.Column('calling_hours_start', sa.String(length=5), server_default='09:00', nullable=False),
        sa.Column('calling_hours_end', sa.String(length=5), server_default='17:00', nullable=False),
        sa.Column('calling_days', postgresql.JSONB(astext_type=sa.Text()), server_default='[1,2,3,4,5]', nullable=False),
        ...
        sa.Column('created_at', sa.DateTime(timezone=True), server_default=sa.text('NOW()'), nullable=False),
        sa.Column('updated_at', sa.DateTime(timezone=True), server_default=sa.text('NOW()'), nullable=False),
        sa.CheckConstraint("status IN ('draft', 'scheduled', 'running', 'paused', 'completed', 'cancelled')", name='campaign_status_check'),
    )
    ...
    op.create_table(
        ...
        sa.Column('contact_id', postgresql.UUID(as_uuid=True), sa.ForeignKey('contacts.id', ondelete='CASCADE'), nullable=False),
        sa.Column('call_log_id', postgresql.UUID(as_uuid=True), sa.ForeignKey('call_logs.id', ondelete='SET NULL'), nullable=True),
        ...
        sa.Column('created_at', sa.DateTime(timezone=True), server_default=sa.text('NOW()'), nullable=False),
        sa.CheckConstraint("status IN ('pending', 'dialing', 'answered', 'completed', 'failed', 'skipped', 'dnc')", name='campaign_call_status_check'),
    )
    ...
    op.create_table(
        ...
        sa.Column('is_active', sa.Boolean(), server_default='true', nullable=False),
        sa.CheckConstraint("source IN ('manual', 'opt_out', 'federal', 'state', 'imported')", name='dnc_source_check'),
    )
    op.create_index('ix_dnc_entries_tenant_id', 'dnc_entries', ['tenant_id'])
    op.create_index('ix_dnc_phone_tenant', 'dnc_entries', ['tenant_id', 'phone_number'], unique=True)
    ...
    op.create_table(
        ...
        sa.Column('created_at', sa.DateTime(timezone=True), server_default=sa.text('NOW()'), nullable=False),
        sa.CheckConstraint("consent_type IN ('call', 'sms', 'recording', 'ai_disclosure')", name='consent_type_check'),
    ...
    # RLS Policies
    all_tables = ['contacts', 'contact_lists', 'contact_list_members', 'campaigns', 'campaign_calls', 'dnc_entries', 'consent_records']
...
def downgrade() -> None:
    for table in ['consent_records', 'dnc_entries', 'campaign_calls', 'campaigns', 'contact_list_members', 'contact_lists', 'contacts']:
        op.drop_table(table)
