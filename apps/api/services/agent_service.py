...
from apps.api.schemas.agent import AgentCreate, AgentUpdate, AgentVersionCreate, AgentVersionUpdate
...
class AgentService:
    ...
    async def create_agent(self, data: AgentCreate) -> Agent:
        agent = Agent(
            tenant_id=self.tenant_id, name=data.name, description=data.description, status="draft"
        )
    ...
    async def set_version_testing(self, version_id: UUID) -> AgentVersion:
        ...
        if version.status != "draft":
            raise HTTPException(
                status_code=400, detail="Only draft versions can be moved to testing"
            )
    ...
    async def publish_version(
        self, agent_id: UUID, version_id: UUID, user_id: UUID
    ) -> AgentVersion:
        agent = await self.get_agent(agent_id)
        version = await self.get_version(version_id)
        ...
        if version.status not in ["draft", "testing"]:
            raise HTTPException(
                status_code=400, detail="Only draft or testing versions can be published"
            )
    ...
    async def rollback_version(
        self, agent_id: UUID, target_version_id: UUID, user_id: UUID
    ) -> AgentVersion:
        agent = await self.get_agent(agent_id)
