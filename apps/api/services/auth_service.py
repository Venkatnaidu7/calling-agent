...
class AuthService:
    ...
    async def register(self, data: RegisterRequest, ip: str = None, user_agent: str = None) -> TokenResponse:
        ...
        # Create session
        user_session = await self.session_repo.create(
            user_id=user.id,
            tenant_id=tenant.id,
    ...
    async def login(self, data: LoginRequest, ip: str = None, user_agent: str = None) -> TokenResponse:
        ...
        if not tenant or tenant.status != "active":
            raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Tenant account is not active")
    ...
    async def refresh_token(self, token: str, ip: str = None, user_agent: str = None) -> TokenResponse:
