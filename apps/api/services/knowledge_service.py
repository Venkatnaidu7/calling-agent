...
from apps.api.schemas.knowledge import KnowledgeBaseCreate, KnowledgeBaseUpdate, DocumentUpload, SearchResult, ChunkResponse
from apps.api.repositories.knowledge_repo import KnowledgeBaseRepository, KnowledgeDocumentRepository, KnowledgeChunkRepository
...
class KnowledgeService:
    ...
    async def list_knowledge_bases(self, skip: int = 0, limit: int = 100) -> Sequence[KnowledgeBase]:
    ...
    async def update_knowledge_base(self, kb_id: uuid.UUID, data: KnowledgeBaseUpdate) -> KnowledgeBase:
    ...
    async def list_documents(self, kb_id: uuid.UUID, skip: int = 0, limit: int = 100) -> Sequence[KnowledgeDocument]:
        return await self.doc_repo.get_all(tenant_id=self.tenant_id, knowledge_base_id=kb_id, skip=skip, limit=limit)
    ...
    async def process_document(self, kb_id: uuid.UUID, doc_id: uuid.UUID) -> KnowledgeDocument:
        ...
        try:
            ...
            raw_text = await self.document_processor.extract_text(content_to_process, doc.source_type)
            ...
            await self.kb_repo.update(tenant_id=self.tenant_id, id=kb.id, chunk_count=new_kb_chunk_count)
    ...
    async def search(self, kb_id: uuid.UUID, query: str, max_results: int = 5, min_relevance: float = 0.7) -> SearchResult:
        await self.get_knowledge_base(kb_id)
