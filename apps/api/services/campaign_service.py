...
from apps.api.schemas.campaign import CampaignCreate, CampaignUpdate, CampaignCallCreate, CampaignCallUpdate
...
class CampaignService:
    ...
    async def update_campaign(self, campaign_id: UUID, data: CampaignUpdate) -> Campaign:
        campaign = await self.campaign_repo.update(campaign_id, **data.model_dump(exclude_unset=True))
        if not campaign:
            raise HTTPException(status_code=404, detail="Campaign not found")
        return campaign
