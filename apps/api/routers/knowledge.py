...
from apps.api.dependencies import (
    get_tenant_context as get_current_user_tenant_id,
    require_roles,
    require_permissions,
)
...
async def search_knowledge_base(
    ...
    if kb_id != data.knowledge_base_id:
        raise HTTPException(
            status_code=400, detail="Path kb_id must match request body knowledge_base_id"
        )
    return await service.search(
        kb_id, data.query, max_results=data.max_results, min_relevance=data.min_relevance
    )
