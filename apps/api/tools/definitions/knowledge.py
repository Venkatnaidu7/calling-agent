...
async def query_knowledge_base(query: str, _context: dict = None, **kwargs) -> dict[str, Any]:
    """
    ...
    try:
        ...
        async with async_session_factory() as session:
            ...
            result = await service.search(
                query=query, kb_id=kb_id, max_results=3, min_relevance=0.6
            )

            if not result.chunks:
                return {
                    "found": False,
                    "information": f"No specific documentation found regarding: {query}",
                }
