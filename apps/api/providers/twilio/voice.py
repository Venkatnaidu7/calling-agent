...
from twilio.twiml.voice_response import VoiceResponse, Connect, Stream, Parameter, Say, Dial
...
class TwilioVoiceProvider(TelephonyProvider):
    ...
    async def initiate_outbound_call(self, to_number, from_number, webhook_url, status_callback_url, machine_detection=False, custom_parameters=None):
        def _create_call():
            call_params = {
                "to": to_number,
                "from_": from_number,
    ...
    async def transfer_call_warm(self, provider_call_id, destination_number, from_number, whisper_message=None):
        def _warm_transfer():
        ...
        try:
            agent_call = await asyncio.to_thread(_warm_transfer)
            return TransferResult(
                success=True,
                provider_call_id=provider_call_id,
                transfer_type="warm",
