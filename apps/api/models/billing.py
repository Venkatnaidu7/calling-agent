...
class Subscription(Base, TenantMixin, TimestampMixin):
    ...
    plan_tier: Mapped[str] = mapped_column(String(50), default="starter", server_default="starter")  # starter, pro, enterprise
    ...
    __table_args__ = (
        CheckConstraint("status IN ('trialing', 'active', 'past_due', 'canceled', 'incomplete')", name='subscription_status_check'),
    )
...
class UsageRecord(Base, TenantMixin):
    ...
    metric: Mapped[str] = mapped_column(String(50), nullable=False)  # voice_minutes, llm_tokens, phone_number, tool_call
    ...
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=datetime.utcnow, server_default="NOW()", index=True)
