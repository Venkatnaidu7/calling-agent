...
class CallLog(Base, TenantMixin, TimestampMixin):
    ...
    call_id: Mapped[str] = mapped_column(String(50), nullable=False, unique=True, index=True)  # our internal call ID
    ...
    phone_number_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("phone_numbers.id", ondelete="SET NULL"), nullable=True, index=True)
