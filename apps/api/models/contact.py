...
class ContactListMember(Base, TenantMixin):
    ...
    contact_list_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("contact_lists.id", ondelete="CASCADE"), nullable=False, index=True)
    ...
    added_at = mapped_column(DateTime(timezone=True), server_default=sa.text('NOW()'), nullable=False)
    __table_args__ = (
        Index('ix_list_member_unique', 'contact_list_id', 'contact_id', unique=True),
    )
