...
from sqlalchemy import String, Integer, Float, Text, Boolean, ForeignKey, CheckConstraint, Index
...
class KnowledgeDocument(Base, TenantMixin, TimestampMixin):
    ...
    source_type: Mapped[str] = mapped_column(String(20), nullable=False)  # text, pdf, docx, url, csv
    ...
    __table_args__ = (
        CheckConstraint("source_type IN ('text', 'pdf', 'docx', 'url', 'csv')", name='doc_source_type_check'),
        CheckConstraint("status IN ('pending', 'processing', 'completed', 'failed')", name='doc_status_check'),
    )
...
class KnowledgeChunk(Base, TenantMixin):
    ...
    metadata_json: Mapped[dict] = mapped_column(JSONB, default=dict, server_default='{}')
    created_at = mapped_column(sa.DateTime(timezone=True), nullable=False, server_default=sa.text('NOW()'))
